...
import (
	...
	v1beta1 "github.com/alibaba/v6d/k8s/api/k8s/v1beta1"
	clientset "github.com/alibaba/v6d/k8s/generated/clientset/versioned"
	clientsetv1beta1 "github.com/alibaba/v6d/k8s/generated/clientset/versioned/typed/k8s/v1beta1"

	_ "github.com/alibaba/v6d/k8s/generated/informers/externalversions/k8s/v1beta1"
	_ "github.com/alibaba/v6d/k8s/generated/listers/k8s/v1beta1"
)

// SchedulerState records the status of current scheduling
type SchedulerState struct {
	state     map[string]map[string]string // { jobname: { pod: nodename }}
	localctl  clientsetv1beta1.LocalObjectInterface
	globalctl clientsetv1beta1.GlobalObjectInterface
}
...
func (ss *SchedulerState) getGlobalObjectsByID(ctx context.Context, objectIds []string) ([]*v1beta1.GlobalObject, error) {
	objects := make([]*v1beta1.GlobalObject, 0)
...
func (ss *SchedulerState) getLocalObjectsBySignatures(ctx context.Context, signatures []string) ([]*v1beta1.LocalObject, error) {
	objects := make([]*v1beta1.LocalObject, 0)
...
func (vs *VineyardScheduling) MakeSchedulerStateForNamespace(namespace string) *SchedulerState {
	if _, ok := vs.state[namespace]; !ok {
		...
		localctl := vs.client.K8sV1beta1().LocalObjects(namespace)
		globalctl := vs.client.K8sV1beta1().GlobalObjects(namespace)
